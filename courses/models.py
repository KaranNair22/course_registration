...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    password_hash = models.CharField(max_length=255, blank=True)  # kept for ER parity; not used by Django
    ...
    # compatibility helpers used by Django templates and admin
    def get_full_name(self):
        return self.full_name

    def get_short_name(self):
        return (self.first_name or self.email) if self.first_name else self.email
...
class Student(models.Model):
    ...
    def __str__(self):
        return f"{self.reg_no} - {self.user.full_name or self.user.email}"
...
class Instructor(models.Model):
    ...
    def __str__(self):
        if self.user:
            name = f"{self.user.full_name}".strip()
            return f"{name} ({self.employee_no})" if name else self.employee_no
...
class CourseSection(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=('course', 'term', 'section_no'), name='unique_course_term_section')
        ]
    ...
    def seats_remaining(self):
        enrolled = self.enrollments.filter(status=Enrollment.Status.ENROLLED).count()
